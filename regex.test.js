...
function ioEmail(email) {
    let regNum = /^[a-zA-Z0-9+-]+@(?:(?:[a-zA-Z0-9-]+\.)?[a-zA-Z]+\.)?(example)\.io$/g;
    let finRes = regNum.test(email);
    return finRes;
}
...
function imagesSearcher( result) {
    let arr = result.match(/[\w-]+\.(jpg|png|jpeg)/g);
    if (arr == null) return [];
    else return arr;
}
